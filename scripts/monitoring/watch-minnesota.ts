...
const EXTRACT_RULE: ExtractRule = {
  field: "electricianReciprocityApplicationFeeUsd",
  pattern: "application and examination fee of \\$(\\d+)",
  transform: "number",
};

const KNOWN_BASELINE = 50;

async function main() {
  const mode = process.argv.includes("--mode=mock") ? "mock" : "live";
  const sourceId = process.argv.find((a) => a.startsWith("--source-id="))?.split("=")[1] ?? "mn-dli-electrician-reciprocity-fee";
  const result = await watchOfficialSource(sourceId, "https://www.dli.mn.gov/workers/electrician-or-electrical-installer/electrical-license-reciprocity", EXTRACT_RULE, KNOWN_BASELINE, mode);
