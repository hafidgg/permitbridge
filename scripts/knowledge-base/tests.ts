...
import { getColoradoElectricianPageData, getAllSingleStateProfessionSlugs, getElectricianStatePageData } from "../../lib/knowledge-base/electrician-state-data";
...
await test("source jurisdiction matches destination context for all real rules (zero Rule 7 warnings on CRITICAL fields specifically — the only ones that gate publication)", () => {
  const files = fs.readdirSync(REAL_TRANSFER_RULES_DIR).filter((f) => f.endsWith(".json"));
  for (const file of files) {
    const rule = loadRealRule(file);
    const issues = validateTransferRule(rule, realResolveSource, realKnownProfessions);
    const jurisdictionIssues = issues.filter((i) => i.rule === "7");
    if (file === "new-york-to-california.json") {
      // Phase 2B.2: all 9 CRITICAL fields were upgraded to genuine
      // registered rn.ca.gov sources this session (verified separately
      // below) — but 6 NON-critical fields (endorsementProcess,
      // reciprocityAgreementExists, temporaryPermitAvailability,
      // otherRequiredFees, processingTime, exceptions) still cite real
      // but unregistered sources from the original Phase 2B research.
      // This is an honest, documented, non-blocking gap (these fields
      // don't gate publication) — not silently hidden by loosening this
      // test to accept it for every file.
      ...
      assertEqual(jurisdictionIssues.length, 6, `${file}: expected exactly 6 known, non-blocking Rule 7 warnings on non-critical fields — a real remaining quality gap for future improvement, not a regression`);
    } else if (file === "georgia-to-illinois.json") {
      // Phase 2B.3: all 9 CRITICAL fields upgraded to genuine registered
      // idfpr.illinois.gov sources this session — 2 NON-critical fields
      // (temporaryPermitAvailability, processingTime) still cite real
      // but unregistered sources from the original Phase 2B research.
      // Same honest, documented, non-blocking pattern as New York->California.
      const criticalFieldsWithIssues = jurisdictionIssues.filter((i) => CRITICAL_TRANSFER_RULE_FIELDS.some((f) => i.message.startsWith(f)));
      assertEqual(criticalFieldsWithIssues, [], `${file}: expected zero jurisdiction/source-resolution issues on CRITICAL fields specifically`);
...
//     Generalization regression tests: Colorado must be completely
//     unaffected by adding Virginia, Virginia's genuinely different
//     rank+state-specific rules must render correctly and separately,
//     and RN must remain untouched throughout.
// ---------------------------------------------------------------------
console.log("\nVirginia Electrician Public Page (Phase 2D.6.3):");
...
await test("[PERMANENT — Phase 2D.6.3] REGRESSION: getColoradoElectricianPageData() (the Phase 2D.4.2 backward-compatible alias) still returns identical Colorado data after generalization — Colorado is completely unaffected by adding Virginia", () => {
  const data = getColoradoElectricianPageData();
  assert(data !== null);
  assertEqual(data!.state, "colorado");
  assertEqual(data!.tiers.map((t) => t.tier).sort(), ["journeyman", "master"]);
});

await test("[PERMANENT — Phase 2D.6.3] getElectricianStatePageData('virginia') returns both real tiers, genuinely present", () => {
  const data = getElectricianStatePageData("virginia");
  assert(data !== null, "real Virginia data with sufficient sourcing must not be withheld");
  assertEqual(data!.tiers.map((t) => t.tier).sort(), ["journeyman", "master"]);
});

await test("[PERMANENT — Phase 2D.6.3] CRITICAL: Virginia Journeyman's reciprocityRules names Kentucky and West Virginia but explicitly EXCLUDES Maryland — the real, rank-specific rule, never generalized from Colorado's simpler uniform-list pattern", () => {
  const data = getElectricianStatePageData("virginia")!;
  const journeyman = data.tiers.find((t) => t.tier === "journeyman")!;
  const text = journeyman.facts.reciprocityRules.value as string;
  assert(text.includes("Kentucky"), "must include real, confirmed Kentucky agreement");
  assert(text.includes("West Virginia"), "must include real, confirmed West Virginia agreement");
  assert(text.toLowerCase().includes("does not cover journeyman") || text.toLowerCase().includes("does not qualify"), "must explicitly state Maryland's Journeyman exclusion, not just omit it silently");
});

await test("[PERMANENT — Phase 2D.6.3] CRITICAL: Virginia Master's reciprocityRules includes all three states — Kentucky, Maryland, AND West Virginia — genuinely different from Journeyman's list, never copied or merged", () => {
  const data = getElectricianStatePageData("virginia")!;
  const master = data.tiers.find((t) => t.tier === "master")!;
  const text = master.facts.reciprocityRules.value as string;
  for (const state of ["Kentucky", "Maryland", "West Virginia"]) {
    assert(text.includes(state), `Master's list must include ${state}`);
  }
  const journeyman = data.tiers.find((t) => t.tier === "journeyman")!;
  assert(journeyman.facts.reciprocityRules.value !== master.facts.reciprocityRules.value, "Journeyman and Master reciprocityRules must be genuinely distinct text, never the same value shared across tiers");
});

await test("[PERMANENT — Phase 2D.6.3] CRITICAL: ULR (Universal License Recognition) is tracked as a field genuinely SEPARATE from reciprocityRules for Virginia — never merged into the bilateral-agreement text, so a reader from a non-listed state isn't told 'no path exists' when ULR may apply", () => {
  const data = getElectricianStatePageData("virginia")!;
  for (const { facts } of data.tiers) {
    assertEqual(facts.universalLicenseRecognitionStatus.value, true, "Virginia's ULR field must be populated true, confirmed via official DPOR source");
    assert(!!facts.universalLicenseRecognitionStatus.sourceUrl, "ULR claim must be traceable to a real source");
    const reciprocityText = (facts.reciprocityRules.value as string).toLowerCase();
    assert(!reciprocityText.includes("ulr covers all states") && !reciprocityText.includes("any state may use ulr"), "reciprocityRules text must not conflate ULR's broader availability into the specific 3-state bilateral list");
  }
});

await test("[PERMANENT — Phase 2D.6.3] every critical field in both Virginia tiers passes the real quality gate — confirmed via direct invocation, not assumed", () => {
  const sources = loadAllSources();
  const sourceByUrl = new Map(sources.map((s) => [s.website, s]));
  const resolveSource = (url: string) => sourceByUrl.get(url);
  const data = getElectricianStatePageData("virginia")!;
  for (const { tier, facts } of data.tiers) {
    const result = isProfessionStateFactsPublishable(facts, resolveSource);
    assertEqual(result.publishable, true, `virginia-${tier} must pass its own real quality gate: ${result.blockingReasons.join("; ")}`);
  }
});
...
await test("[PERMANENT — Phase 2D.6.3] RN REGRESSION: getPublicTransferRule still resolves every real RN pair exactly as before — the generalized electrician branch never intercepts or shadows an RN lookup", () => {
  for (const slug of getAllPublicTransferRuleSlugs()) {
    const rule = getPublicTransferRule(slug.profession, slug.transfer);
    assert(!!rule, `${slug.profession}/${slug.transfer} must still resolve to a real rule, unaffected by Phase 2D.6.3`);
  }
});

await test("[PERMANENT — Phase 2D.6.3] no RN content leakage: neither Virginia's nor Colorado's page data contains any reference to 'Registered Nurse', 'RN', or nursing-specific terminology — confirmed by direct content inspection, not just route isolation", () => {
  for (const state of ["colorado", "virginia"]) {
    const data = getElectricianStatePageData(state)!;
    for (const { facts } of data.tiers) {
      const allText = JSON.stringify(facts).toLowerCase();
      assert(!allText.includes("registered nurse") && !allText.includes("nursys") && !allText.includes("nclex"), `${state}: electrician page data must contain zero RN-specific terminology`);
    }
  }
});
...
await test("[PERMANENT] getAllSingleStateProfessionSlugs() includes electrician/arkansas, and both arkansas tiers (journeyman, master) pass the real quality gate", () => {
  const slugs = getAllSingleStateProfessionSlugs();
  assert(slugs.some((s) => s.slug === "arkansas"), "arkansas must appear in the whitelist output — its fact files must pass isProfessionStateFactsPublishable() for both tiers");
  const data = getElectricianStatePageData("arkansas");
  assert(data !== null, "arkansas fact files must pass the quality gate for both journeyman and master tiers");
  assertEqual(data!.tiers.map((t) => t.tier).sort(), ["journeyman", "master"]);
});

await test("[PERMANENT] getAllSingleStateProfessionSlugs() includes electrician/minnesota, and both minnesota tiers (journeyman, master) pass the real quality gate", () => {
  const slugs = getAllSingleStateProfessionSlugs();
  assert(slugs.some((s) => s.slug === "minnesota"), "minnesota must appear in the whitelist output — its fact files must pass isProfessionStateFactsPublishable() for both tiers");
  const data = getElectricianStatePageData("minnesota");
  assert(data !== null, "minnesota fact files must pass the quality gate for both journeyman and master tiers");
  assertEqual(data!.tiers.map((t) => t.tier).sort(), ["journeyman", "master"]);
});

await test("[PERMANENT] getAllSingleStateProfessionSlugs() includes electrician/utah, and both utah tiers (journeyman, master) pass the real quality gate", () => {
  const slugs = getAllSingleStateProfessionSlugs();
  assert(slugs.some((s) => s.slug === "utah"), "utah must appear in the whitelist output — its fact files must pass isProfessionStateFactsPublishable() for both tiers");
  const data = getElectricianStatePageData("utah");
  assert(data !== null, "utah fact files must pass the quality gate for both journeyman and master tiers");
  assertEqual(data!.tiers.map((t) => t.tier).sort(), ["journeyman", "master"]);
});

await test("[PERMANENT] getAllSingleStateProfessionSlugs() includes electrician/wyoming, and both wyoming tiers (journeyman, master) pass the real quality gate", () => {
  const slugs = getAllSingleStateProfessionSlugs();
  assert(slugs.some((s) => s.slug === "wyoming"), "wyoming must appear in the whitelist output — its fact files must pass isProfessionStateFactsPublishable() for both tiers");
  const data = getElectricianStatePageData("wyoming");
  assert(data !== null, "wyoming fact files must pass the quality gate for both journeyman and master tiers");
  assertEqual(data!.tiers.map((t) => t.tier).sort(), ["journeyman", "master"]);
});

await test("[PERMANENT] CRITICAL: Wyoming's Journeyman (17 states) and Master (3 states) reciprocal-state lists are genuinely different and were independently verified — the 3 overlap states (Idaho, South Dakota, Iowa) carry tier-specific exam scores (70% vs 75%) and Master's Journeyman-history prerequisite, never assumed identical across tiers", () => {
  const data = getElectricianStatePageData("wyoming")!;
  const journeyman = data.tiers.find((t) => t.tier === "journeyman")!;
  const master = data.tiers.find((t) => t.tier === "master")!;
  ...
  for (const state of ["Alaska", "Arkansas", "Colorado", "Idaho", "Maine", "Minnesota", "Montana", "Nebraska", "New Hampshire", "New Mexico", "North Dakota", "Oklahoma", "Oregon", "South Dakota", "Texas", "Iowa", "Utah"]) {
    assert(jText.includes(state), `Journeyman's reciprocal-state list must include the real, documented state: ${state}`);
  }
  for (const state of ["Idaho", "South Dakota", "Iowa"]) {
    assert(mText.includes(state), `Master's reciprocal-state list must include the real, documented state: ${state}`);
  }
  const mQuotedList = mText.match(/following states: ([^.]+)\./)?.[1];
  assert(!!mQuotedList, "Master's reciprocityRules must contain the quoted official state-list sentence");
  assert(!mQuotedList!.includes("Colorado") && !mQuotedList!.includes("Texas") && !mQuotedList!.includes("Oregon"), "Master's quoted state list must never be contaminated with Journeyman-only states — mentioning Oregon/Utah elsewhere in the narrative to explain their EXCLUSION from Master is fine, but they must not appear inside the actual quoted list itself");
  assert(jText !== mText, "Journeyman and Master reciprocityRules must be genuinely distinct text, never shared across tiers");

  const jExam = journeyman.facts.requiredExams.value as string;
  const mExam = master.facts.requiredExams.value as string;
  const jScore = jExam.match(/passed with a score of (\d+)% or greater/)?.[1];
  const mScore = mExam.match(/passed with a score of (\d+)% or greater/)?.[1];
  assertEqual(jScore, "70", "Journeyman's own exam-score requirement must be 70%");
  assertEqual(mScore, "75", "Master's own exam-score requirement must be 75%, genuinely higher than Journeyman's — never assumed equal just because both tiers reciprocate with Idaho/South Dakota/Iowa");
});
...
await test("[PERMANENT] sitemap.ts derives Arkansas automatically via the same whitelist function — no manually-written '/electrician/arkansas' URL string exists", () => {
  const sitemapSource = fs.readFileSync(path.join(process.cwd(), "app", "sitemap.ts"), "utf-8");
  assert(!sitemapSource.includes('"/electrician/arkansas"') && !sitemapSource.includes("'/electrician/arkansas'"), "no literal, manually-typed URL string for Arkansas may exist in sitemap.ts");
});

await test("[PERMANENT] sitemap.ts derives Minnesota automatically via the same whitelist function — no manually-written '/electrician/minnesota' URL string exists", () => {
  const sitemapSource = fs.readFileSync(path.join(process.cwd(), "app", "sitemap.ts"), "utf-8");
  assert(!sitemapSource.includes('"/electrician/minnesota"') && !sitemapSource.includes("'/electrician/minnesota'"), "no literal, manually-typed URL string for Minnesota may exist in sitemap.ts");
});

await test("[PERMANENT] sitemap.ts derives Utah automatically via the same whitelist function — no manually-written '/electrician/utah' URL string exists", () => {
  const sitemapSource = fs.readFileSync(path.join(process.cwd(), "app", "sitemap.ts"), "utf-8");
  assert(!sitemapSource.includes('"/electrician/utah"') && !sitemapSource.includes("'/electrician/utah'"), "no literal, manually-typed URL string for Utah may exist in sitemap.ts");
});

await test("[PERMANENT] sitemap.ts derives Wyoming automatically via the same whitelist function — no manually-written '/electrician/wyoming' URL string exists", () => {
  const sitemapSource = fs.readFileSync(path.join(process.cwd(), "app", "sitemap.ts"), "utf-8");
  assert(!sitemapSource.includes('"/electrician/wyoming"') && !sitemapSource.includes("'/electrician/wyoming'"), "no literal, manually-typed URL string for Wyoming may exist in sitemap.ts");
});

await test("[PERMANENT] REGRESSION: Colorado, Virginia, Texas, Arkansas, Minnesota, and Utah electrician data are completely unaffected by adding Wyoming", () => {
  assert(getColoradoElectricianPageData() !== null);
  for (const state of ["virginia", "texas", "arkansas", "minnesota", "utah"]) {
    assert(getElectricianStatePageData(state) !== null, `${state} must remain publishable after Wyoming was added`);
  }
});

await test("[PERMANENT] no RN content leakage in Wyoming's electrician page data, and no cross-tier leakage between Wyoming journeyman and master", () => {
  const data = getElectricianStatePageData("wyoming")!;
  for (const { facts } of data.tiers) {
    const allText = JSON.stringify(facts).toLowerCase();
    assert(!allText.includes("registered nurse") && !allText.includes("nursys") && !allText.includes("nclex"), "Wyoming electrician page data must contain zero RN-specific terminology");
  }
  const journeyman = data.tiers.find((t) => t.tier === "journeyman")!;
  const master = data.tiers.find((t) => t.tier === "master")!;
  assert((master.facts.requiredExperience.value as string).toLowerCase().includes("licensed journeyman"), "Master's requiredExperience must retain its own genuinely different Journeyman-history prerequisite, never copied from Journeyman's simpler requirement");
  assert(journeyman.facts.requiredExperience.value !== master.facts.requiredExperience.value, "Journeyman and Master requiredExperience must be genuinely distinct text, never shared across tiers");
});

await test("[PERMANENT] no RN content leakage in Arkansas's electrician page data, and no cross-tier leakage between Arkansas journeyman and master", () => {
  const data = getElectricianStatePageData("arkansas")!;
  for (const { facts } of data.tiers) {
    const allText = JSON.stringify(facts).toLowerCase();
    assert(!allText.includes("registered nurse") && !allText.includes("nursys") && !allText.includes("nclex"), "arkansas: electrician page data must contain zero RN-specific terminology");
  }
  const journeyman = data.tiers.find((t) => t.tier === "journeyman")!.facts;
  const master = data.tiers.find((t) => t.tier === "master")!.facts;
  assert(journeyman.reciprocityRules.value !== master.reciprocityRules.value, "Arkansas journeyman and master reciprocityRules must genuinely differ — the Journeyman (17-state) and Master (4-state, asymmetric) reciprocal lists are materially different, not the same text duplicated across tiers");
});

await test("[PERMANENT] no RN content leakage in Minnesota's electrician page data, and no cross-tier leakage between Minnesota journeyman and master", () => {
  const data = getElectricianStatePageData("minnesota")!;
  for (const { facts } of data.tiers) {
    const allText = JSON.stringify(facts).toLowerCase();
    assert(!allText.includes("registered nurse") && !allText.includes("nursys") && !allText.includes("nclex"), "minnesota: electrician page data must contain zero RN-specific terminology");
  }
  const journeyman = data.tiers.find((t) => t.tier === "journeyman")!.facts;
  const master = data.tiers.find((t) => t.tier === "master")!.facts;
  assert(journeyman.reciprocityRules.value !== master.reciprocityRules.value, "Minnesota journeyman and master reciprocityRules must genuinely differ — the 9-state Journeyworker list and the 4-state Master list are materially different, not the same text duplicated across tiers");
  assertEqual(journeyman.requiredExams.confidenceLevel, "verified", "Minnesota journeyman requiredExams must have confidenceLevel 'verified' (positive statutory exam exemption)");
  assert(journeyman.requiredExams.status !== "needs_review", "Minnesota journeyman requiredExams must NOT be needs_review — unlike Arkansas's inferred equivalent, Minn. Stat. 326B.33 Subd. 20 states the exam exemption positively");
  assertEqual(master.requiredExams.confidenceLevel, "verified", "Minnesota master requiredExams must have confidenceLevel 'verified' (positive statutory exam exemption)");
  assert(master.requiredExams.status !== "needs_review", "Minnesota master requiredExams must NOT be needs_review — unlike Arkansas's inferred equivalent");
});

await test("[PERMANENT] no RN content leakage in Utah's electrician page data, and no cross-tier leakage between Utah journeyman and master", () => {
  const data = getElectricianStatePageData("utah")!;
  for (const { facts } of data.tiers) {
    const allText = JSON.stringify(facts).toLowerCase();
    assert(!allText.includes("registered nurse") && !allText.includes("nursys") && !allText.includes("nclex"), "utah: electrician page data must contain zero RN-specific terminology");
  }
  const journeyman = data.tiers.find((t) => t.tier === "journeyman")!.facts;
  const master = data.tiers.find((t) => t.tier === "master")!.facts;
  assert(journeyman.reciprocityRules.value !== master.reciprocityRules.value, "Utah journeyman and master reciprocityRules must genuinely differ — the Oregon agreement's journeyman and master/general-supervising terms are separately stated, not the same text duplicated across tiers");
